...
import { Db, auth } from '../../Database/init-firebase';
...
const AddNewStudents = () => {
  ...
  const handleValidSubmit = async(e, data) => {

    console.log(data);

    try {
      const rslt = await createUserWithEmailAndPassword(auth, data.email, data.number);

      console.log(rslt);
      
    //  const res = await addDoc(collection(Db, "STUDENTS"), { data});
     const res = await setDoc(doc(Db, "STUDENTS", " CnrFqZ5iYuZA5Ff1f1Ws5o0bQXk1"), { data, timeStamp: serverTimestamp() });
