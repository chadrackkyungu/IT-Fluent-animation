...
const AddTeachers = () => {
  ...
  const handleValidSubmit = async(e, std_Input) => {
    ...
    const allfield = {TeacherName, TeacherLastName, TeacherIdNumber,date_of_birth,  email, number, gender, religion, address,birthCountry, img,  timeStamp: serverTimestamp()};

    try {
      const rslt = await createUserWithEmailAndPassword(auth, std_Input.email, std_Input.TeacherIdNumber); 
     const res = await setDoc(doc(Db, "TEACHERS", rslt.user.uid), { allfield });
     successTost_addStd();
    } catch (error) {
      errorTost()
    }
