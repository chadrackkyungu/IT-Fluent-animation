...
const AddNotice = () => {
  ...
  const handleValidSubmit = async(e, std_Input) => {
    const { title,postedBy,details,date} = std_Input;
    const allfield = {title,postedBy,details,date,  timeStamp: serverTimestamp()};
    try {
      await addDoc(collection(Db, "NOTICE"), { allfield });
     successTost_addStd();
    } catch (error) {
      errorTost()
    }
